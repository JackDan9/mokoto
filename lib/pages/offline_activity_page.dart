...
import '../api/api.dart';
import '../components/common_web_page.dart';
...
class OfflineActivityPageState extends State<OfflineActivityPage> {
  ...
  Widget getJackDanBody() {
    if (jackDanData == null) {
      return Center(child: CircularProgressIndicator());
    } else if (jackDanData.length == 0) {
      return Center(child: Text("暂无数据"));
    } else {
      return ListView.builder(itemBuilder: _renderJackDanRow, itemCount: jackDanData.length);
    }
  }
  ...
  Widget _renderJackDanRow(BuildContext ctx, int i) {
    Map itemData = jackDanData[i];
    return InkWell(
      child: getCard(itemData),
      onTap: () {
        _showDetail(itemData['detailUrl']);
